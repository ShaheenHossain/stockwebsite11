        ...
        for (var k in variant_ids) {
            if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
                ...
                if(_.contains(['always', 'threshold'], info['inventory_availability'])) {
                    ...
                    if(info['virtual_available']>0 && info.old_lot){
                        var date_array=info.old_lot.split(' ')[0].split('-');
                        var string_date = date_array[2]+'-'+date_array[1]+'-'+date_array[0];
                        $('span.lot').html($('span.lot').html().split(':')[0]+': '+string_date)
                        $('span.lot').fadeIn()
